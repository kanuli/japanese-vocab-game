...
if(prebuilt&&prebuilt.version===REQUIRED&&Number(prebuilt.generatedCount||prebuilt.loadedCount||0)>=MIN&&prebuilt.jlptRecalibration?.status==="complete"&&prebuilt.jlptRecalibration?.version==="20260826-teacher-v4-exact-audit"){
  window.ADVANCED_WORDS_META={...prebuilt,prebuilt:true,audited:true,teacherAudit:true};
  return;
}
const msg=`⚠️ 日文老師審核詞庫／JLPT 校準資料未載入或版本不符（需要 ${REQUIRED} / ${MIN}+ 詞）。已停用舊式瀏覽器重建，避免重新引入同音、錯義或錯誤 N1 fallback。`;
