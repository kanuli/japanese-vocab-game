...
// Prefer the validated repo-hosted bundle; cached/browser construction remains a fallback.
(function(){"use strict";
const prebuilt=window.ADVANCED_WORDS_BUNDLE_META;
if(prebuilt&&Number(prebuilt.generatedCount||prebuilt.loadedCount||0)>=9500){window.ADVANCED_WORDS_META={...prebuilt,prebuilt:true};return}
const V="large-vocab-20260814-v7",M=`jpquiz_${V}_meta`,P=`jpquiz_${V}_chunk_`;
...
setTimeout(async()=>{try{for(const f of["large_vocab_sources.js","large_vocab_corefix.js","large_vocab_matchers.js","large_vocab_build.js"])await load(`./${f}?v=20260814v7`);await window.JPVocabLarge.build()}catch(e){console.error(e);const x=document.getElementById("advStatus");if(x)x.textContent=`⚠️ 大型詞庫載入失敗：${e.message}`}},0);
})();
