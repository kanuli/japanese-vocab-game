...
var CFG={
 voicevox:{label:'🎭 VOICEVOX｜伺服器預錄',catalog:'./word-voicevox-catalog.json?v=3',group:'speakers'},
 supertonic3:{label:'✨ Supertonic 3｜伺服器預錄',catalog:'./word-supertonic3-catalog.json?v=2',group:'voices'},
 aivis:{label:'💠 AivisSpeech / Style-Bert-VITS｜伺服器預錄',catalog:'./word-aivis-catalog.json?v=2',group:'voices'},
...
function speakState(partial){
  return publishSpeak(Object.assign(emptySpeak(), W.lastSpeak||{}, partial||{}));
}
...
function lookupHosted(c,w){if(!c||!c.words||!w)return null;var exact=c.words[wordKey(w)];if(exact)return exact;var r=normReading(w.reading);if(!r)return null;return readingIndex(c)[r]||null;}
...
async function rangeBytes(bundle,offset,size){var end=offset+size-1,urls=(guard().isMobile?[bundle.hfUrl,bundle.githubUrl,bundle.url]:[bundle.githubUrl,bundle.hfUrl,bundle.url]).filter(Boolean),last=null;for(var i=0;i<urls.length;i++){try{var r=await fetch(urls[i],{headers:{Range:'bytes='+offset+'-'+end},cache:'force-cache'});var len=Number(r.headers.get('content-length')||0);if(r.status!==206&&!(r.status===200&&len===size))throw new Error('Range HTTP '+r.status+' length '+len);var b=await r.arrayBuffer();if(b.byteLength!==size)throw new Error('Range size '+b.byteLength+'/'+size);rangeBytes.lastUrl=urls[i];rangeBytes.lastStatus=r.status;return b;}catch(e){last=e;}}throw last||new Error('音訊下載失敗');}
async function playHosted(eng,w){var c=await loadCatalog(eng);if(!c||c.status!=='ready'||!w){speakState({provider:eng,hostedMiss:true,hostedHit:false,catalog:CFG[eng]&&CFG[eng].catalog||''});return null;}var lookup=lookupHosted(c,w);if(!lookup){speakState({provider:eng,hostedMiss:true,hostedHit:false,requestedReading:String(w.reading||''),catalog:CFG[eng]&&CFG[eng].catalog||''});return null;}var id=lookup[0],shard=String(lookup[1]),all=keys(c),sel=el('voice'),requested=sel&&sel.value;var key;if(requested&&requested!=='random'){if(all.indexOf(requested)<0){speakState({provider:eng,hostedMiss:true,hostedHit:false,selectedVoice:requested,requestedReading:String(w.reading||''),catalog:CFG[eng]&&CFG[eng].catalog||'',error:'selected voice not in catalog'});return null;}key=requested;}else{key=all[Math.floor(Math.random()*all.length)];}if(all.indexOf(key)<0)key=all[0];if(!key){speakState({provider:eng,hostedMiss:true,hostedHit:false});return null;}var idx=await loadIndex(eng,key),bundle=idx.bundles&&idx.bundles[shard],member=bundle&&bundle.members&&bundle.members[id];if(!bundle||!member){speakState({provider:eng,hostedMiss:true,hostedHit:false,key:key,requestedReading:String(w.reading||'')});return null;}status('正在讀取 '+CFG[eng].label.replace(/^[^ ]+ /,'')+'：'+String(w.reading||'')+'…');var bytes=await rangeBytes(bundle,Number(member[0]),Number(member[1]));var usedUrl=rangeBytes.lastUrl||'';speakState({provider:eng,hostedHit:true,hostedMiss:false,fallbackUsed:false,key:key,selectedVoice:requested||key,requestedReading:String(w.reading||''),resolvedAsset:String(bundle.githubUrl||bundle.hfUrl||bundle.url||''),resolvedUrl:usedUrl,catalog:CFG[eng]&&CFG[eng].catalog||'',httpStatus:rangeBytes.lastStatus||0,pending:false});stopHosted();var speed=Number(el('speed')&&el('speed').value||1);if(!Number.isFinite(speed)||speed<=0)speed=1;try{if(guard().isMobile&&guard().playHostedBytes){await guard().playHostedBytes(bytes,speed);}else{hostedBlobUrl=URL.createObjectURL(new Blob([bytes],{type:'audio/mpeg'}));await new Promise(function(resolve,reject){var a=hostedAudio=new Audio(hostedBlobUrl),done=false;a.playbackRate=speed;function finish(ok,e){if(done)return;done=true;a.onended=a.onerror=null;if(hostedAudio===a)hostedAudio=null;ok?resolve():reject(e||new Error('播放失敗'));}a.onended=function(){finish(true);};a.onerror=function(){finish(false,new Error('預錄音播放失敗'));};setTimeout(function(){finish(false,new Error('playback timeout'));},4000);var p=a.play();if(p&&p.catch)p.catch(function(e){finish(false,e);});});}speakState({playbackSuccess:true,playbackFailure:false,fallbackUsed:false,hostedHit:true,pending:false});}catch(playErr){speakState({playbackSuccess:false,playbackFailure:true,pending:false,hostedHit:true,fallbackUsed:false,error:String(playErr&&playErr.message||playErr)});return{engine:eng,key:key,url:usedUrl,playbackFailure:true};}var meta=group(c)[key]||{};status('✅ '+voiceName(meta,key)+' 已播放。');return{engine:eng,key:key,url:usedUrl};}
...
async function speakDevice(text){var voices=japaneseDeviceVoices();if(!voices.length)throw new Error('此裝置沒有 Japanese voice');var sel=el('voice'),raw=sel&&sel.value,idx=Number(raw);var voice=Number.isFinite(idx)&&voices[idx]?voices[idx]:voices[0];stopDevice();speakState({provider:'device',fallbackUsed:true,hostedHit:false,hostedMiss:true,requestedReading:String(text||''),selectedVoice:voice.name,key:voice.name});await new Promise(function(resolve,reject){var u=deviceUtterance=new SpeechSynthesisUtterance(String(text||''));u.lang='ja-JP';u.voice=voice;var s=Number(el('speed')&&el('speed').value||1);u.rate=Number.isFinite(s)&&s>0?s:1;u.onend=function(){deviceUtterance=null;resolve();};u.onerror=function(e){deviceUtterance=null;reject(e.error||new Error('裝置語音失敗'));};speechSynthesis.speak(u);});speakState({provider:'device',fallbackUsed:true,playbackSuccess:true,playbackFailure:false,hostedHit:false});status('✅ 裝置日語聲線：'+voice.name);return{engine:'device',key:voice.name};}
async function hostedFallback(text,w,from){var order=['supertonic3','voicevox','aivis'].filter(function(x){return x!==from;});for(var i=0;i<order.length;i++){var eng=order[i];status('伺服器預錄暫未命中，正在改用 '+CFG[eng].label+'…');try{var out=await playHosted(eng,w);if(out){speakState({fallbackUsed:false,hostedHit:true,provider:eng,key:out.key});return out;}}catch(e){console.warn(eng+' hosted fallback failed',e);}}status('⚠️ 伺服器預錄暫時無法使用，改用裝置 Japanese voice。');speakState({hostedMiss:true,hostedHit:false,fallbackUsed:true});return speakDevice(text);}
W.speak=async function(text,overrideWord){var eng=engine(),w=wordFor(text,overrideWord);W.__speakSeq=(W.__speakSeq||0)+1;publishSpeak(Object.assign(emptySpeak(),{requestedReading:String(text||''),provider:eng,selectedVoice:(el('voice')&&el('voice').value)||'',pending:true,seq:W.__speakSeq}));stopHosted();if(eng==='device')return speakDevice(text);if(CFG[eng]&&CFG[eng].catalog){try{var out=await playHosted(eng,w);if(out){speakState({fallbackUsed:false,hostedHit:true,playbackSuccess:!out.playbackFailure,playbackFailure:!!out.playbackFailure,pending:false,provider:eng,key:out.key,resolvedUrl:out.url||(W.lastSpeak&&W.lastSpeak.resolvedUrl)||''});return out;}}catch(e){console.warn(eng+' hosted failed',e);speakState({error:String(e&&e.message||e)});}return hostedFallback(text,w,eng);}return hostedFallback(text,w,'');};
...
window.WORD_AUDIO_MULTI_VOICE={version:5,engines:['voicevox','supertonic3','aivis','device'],sync:syncUI,loadCatalog:loadCatalog,stop:stopAll,hostedOnly:true,mobileSafe:true,auditionExactVoice:true};
installUI();
})();
