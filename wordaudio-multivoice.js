...
async function speakST3(text){if(typeof oldSpeak!=='function')throw new Error('Supertonic 3 browser fallback unavailable');var v=el('voice');if(v&&!/^([FM][1-5]|random)$/.test(v.value))v.value='F3';return oldSpeak(String(text||''));}
...
async function fallbackST3(text,from){status((from==='voicevox'?'VOICEVOX':from==='aivis'?'AivisSpeech / Style-Bert-VITS':'Supertonic 3 預錄庫')+' 此單字暫無可用伺服器錄音，使用 Supertonic 3 瀏覽器備援。');return speakST3(text);}
W.speak=async function(text,overrideWord){var eng=engine(),w=wordFor(text,overrideWord);stopHosted();if(eng==='device')return speakDevice(text);if(eng==='supertonic3'){
  try{var h=await playHosted('supertonic3',w);if(h)return h;}catch(e){console.warn('Supertonic 3 hosted failed',e);}return speakST3(text);
 }
 if(eng==='voicevox'||eng==='aivis'){
  try{var out=await playHosted(eng,w);if(out)return out;}catch(e){console.warn(eng+' hosted failed',e);}return fallbackST3(text,eng);
 }
 return speakST3(text);
};
...
function installUI(){var voice=el('voice');if(!voice)return;var parent=voice.parentNode;if(!el('audioEngine')){var wrap=document.createElement('div');wrap.className='field';wrap.style.marginTop='10px';wrap.innerHTML='<label for="audioEngine">語音來源</label><select id="audioEngine"><option value="voicevox">'+CFG.voicevox.label+'</option><option value="supertonic3" selected>'+CFG.supertonic3.label+'</option><option value="aivis">'+CFG.aivis.label+'</option><option value="device">'+CFG.device.label+'</option></select>';parent.parentNode.insertBefore(wrap,parent);el('audioEngine').onchange=syncUI;}
 var title=document.querySelector('#setup aside h2');if(title)title.textContent='🔊 單字語音｜VOICEVOX / Supertonic 3 / AivisSpeech';var note=document.querySelector('#setup aside .notice');if(note)note.textContent='三種主要日語引擎共用本站的單字聲線資料庫。伺服器預錄未完成或缺漏時會使用 Supertonic 3 瀏覽器備援；不需要連接本機 IP。';var sample=el('sampleVoice');if(sample){sample.onclick=async function(){sample.disabled=true;sampleStatus('正在準備試聽…');try{var w=W.words&&W.words.length?W.words[0]:null,text=w?w.reading:'こんにちは';var used=await W.speak(text,w);sampleStatus('✅ 試聽完成：'+(used&&used.engine?used.engine:engine())+'｜'+text);}catch(e){sampleStatus('⚠️ 試聽失敗：'+(e&&e.message?e.message:String(e)));}finally{sample.disabled=false;}};}
 if('speechSynthesis' in window)speechSynthesis.onvoiceschanged=function(){if(engine()==='device')renderDevice();};syncUI();}
window.WORD_AUDIO_MULTI_VOICE={version:1,engines:['voicevox','supertonic3','aivis','device'],sync:syncUI,loadCatalog:loadCatalog,stop:stopAll};
installUI();
})(window.WA);
