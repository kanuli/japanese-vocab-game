...
W.speak=async function(text,overrideWord){var eng=engine(),w=wordFor(text,overrideWord);publishSpeak(Object.assign(emptySpeak(),{requestedReading:String(text||''),provider:eng,selectedVoice:(el('voice')&&el('voice').value)||''}));stopHosted();if(eng==='device')return speakDevice(text);if(CFG[eng]&&CFG[eng].catalog){try{var out=await playHosted(eng,w);if(out){speakState({fallbackUsed:false,hostedHit:true,playbackSuccess:true,provider:eng,key:out.key,resolvedUrl:out.url||(W.lastSpeak&&W.lastSpeak.resolvedUrl)||''});return out;}}catch(e){console.warn(eng+' hosted failed',e);speakState({error:String(e&&e.message||e)});}return hostedFallback(text,w,eng);}return hostedFallback(text,w,'');};
