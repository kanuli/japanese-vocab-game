...
// src/supertonic-helper.js
var AVAILABLE_LANGS = ["en", "ko", "ja", "ar", "bg", "cs", "da", "de", "el", "es", "et", "fi", "fr", "hi", "hr", "hu", "id", "it", "lt", "lv", "nl", "pl", "pt", "ro", "ru", "sk", "sl", "sv", "tr", "uk", "vi", "na"];
...
var UnicodeProcessor = class {
  ...
  preprocessText(text, lang) {
    ...
    for (const [k2, v] of Object.entries(replacements)) {
      text = text.replaceAll(k2, v);
    }
    text = text.replace(/[♥☆♡©\\]/g, "");
    ...
    for (const [k2, v] of Object.entries(exprReplacements)) {
      text = text.replaceAll(k2, v);
    }
    text = text.replace(/ ,/g, ",");
    text = text.replace(/ \./g, ".");
...
var TextToSpeech = class {
  ...
  async _infer(textList, langList, style, totalStep, speed = 1.05, progressCallback = null) {
    ...
    const textIdsTensor = new Ke("int64", textIdsFlat, textIdsShape);
    const textMaskFlat = new Float32Array(textMask.flat(2));
    const textMaskShape = [bsz, 1, textMask[0][0].length];
    const textMaskTensor = new Ke("float32", textMaskFlat, textMaskShape);
    ...
    let { xt: xt2, latentMask } = this.sampleNoisyLatent(
      duration,
      this.sampleRate,
    ...
    const latentMaskTensor = new Ke("float32", latentMaskFlat, latentMaskShape);
    const totalStepArray = new Float32Array(bsz).fill(totalStep);
    const totalStepTensor = new Ke("float32", totalStepArray, [bsz]);
    for (let step = 0; step < totalStep; step++) {
      if (progressCallback) {
        progressCallback(step + 1, totalStep);
      }
      const currentStepArray = new Float32Array(bsz).fill(step);
      const currentStepTensor = new Ke("float32", currentStepArray, [bsz]);
      const xtFlat = new Float32Array(xt2.flat(2));
      const xtShape = [bsz, xt2[0].length, xt2[0][0].length];
      const xtTensor = new Ke("float32", xtFlat, xtShape);
      ...
      const latentDim = xt2[0].length;
      const latentLen = xt2[0][0].length;
      xt2 = [];
      let idx = 0;
      for (let b = 0; b < bsz; b++) {
        const batch = [];
    ...
    const finalXtFlat = new Float32Array(xt2.flat(2));
    const finalXtShape = [bsz, xt2[0].length, xt2[0][0].length];
    const finalXtTensor = new Ke("float32", finalXtFlat, finalXtShape);
...
async function loadVoiceStyle(voiceStylePaths, verbose = false) {
  ...
  const ttlTensor = new Ke("float32", ttlFlat, ttlShape);
  const dpTensor = new Ke("float32", dpFlat, dpShape);
  if (verbose) {
...
async function loadOnnx(onnxPath, options) {
  const session = await vf.create(onnxPath, options);
  return session;
}
async function loadTextToSpeech(onnxDir, sessionOptions = {}, progressCallback = null) {
...
async function getStyle(voice) {
  const id2 = VOICES.includes(voice) ? voice : "F1";
  if (styles.has(id2)) return styles.get(id2);
  const st2 = await loadVoiceStyle([`${VOICE_BASE}/${id2}.json`]);
  styles.set(id2, st2);
  return st2;
}
async function synthesize(text, { voice = "F1", speed = 1, totalSteps = 5 } = {}) {
  ...
  const result = await tts.call(String(text || ""), "ja", style, totalSteps, Math.max(0.7, Math.min(2, speed)), 0.12);
