...
d=json.loads(CAT.read_text(encoding='utf-8'));words=d['words'];sc=int(d['shardCount']);labels={'F1':'🌙 沉穩低柔女聲（F1）','F2':'🌸 明亮活潑女聲（F2）','F3':'🎙️ 專業播音女聲（F3）','F4':'✨ 清晰自信女聲（F4）','F5':'💕 溫柔療癒女聲（F5）','M1':'⚡ 活力自信男聲（M1）','M2':'🌑 低沉穩重男聲（M2）','M3':'🧭 權威專業男聲（M3）','M4':'🙂 柔和親切男聲（M4）','M5':'📖 溫暖舒緩男聲（M5）'}
voices={}
for v,label in labels.items():
 ...
 if len(seen)!=22333:raise SystemExit(f'{v}: expected 22333 IDs, got {len(seen)}')
 idx={'version':1,'engine':'supertonic-3','voice':v,'label':label,'wordCount':22333,'shardCount':sc,'bundles':bundles}
 ip=Path(f'word-supertonic3-{v}-index.json');ip.write_text(json.dumps(idx,ensure_ascii=False,separators=(',',':'))+'\n',encoding='utf-8')
 voices[v]={'label':label,'indexUrl':f'./{ip.name}?v=1'}
out={'version':1,'status':'ready','engine':'supertonic-3','storage':'github-releases+hf-range-bundles','wordCount':22333,'voiceCount':10,'recordingCount':223330,'shardCount':sc,'words':words,'voices':voices}
