...
new_prepare='''async function prepareRef(play=true){
...
'''
start=s.find('async function prepareRef(play=true){')
end=s.find("$('#play').onclick", start)
assert start>=0 and end>start, f'prepareRef region not found: {start}, {end}'
