...
d=json.loads(CAT.read_text(encoding='utf-8'));model=json.loads(MODEL.read_text(encoding='utf-8'));words=d['words'];sc=int(d['shardCount']);word_count=int(d['wordCount']);voices={}
...
for st in model['styles']:
 ...
 if len(seen)!=word_count:raise SystemExit(f'{k}: expected {word_count} IDs, got {len(seen)}')
 idx={'version':2,'engine':'aivisspeech-style-bert-vits2','voice':k,'speaker':st['speaker'],'style':st['style'],'styleId':st['styleId'],'displayName':f"{st['speaker']}｜{st['style']}",'modelName':model['modelName'],'modelVersion':model['modelVersion'],'modelArchitecture':model['modelArchitecture'],'license':model['license'],'licenseSha256':model['licenseSha256'],'wordCount':word_count,'shardCount':sc,'bundles':bundles}
 ip=Path(f'word-aivis-{k}-index.json');ip.write_text(json.dumps(idx,ensure_ascii=False,separators=(',',':'))+'\n',encoding='utf-8')
 voices[k]={'speaker':st['speaker'],'style':st['style'],'displayName':idx['displayName'],'modelName':model['modelName'],'modelVersion':model['modelVersion'],'modelArchitecture':model['modelArchitecture'],'license':model['license'],'licenseSha256':model['licenseSha256'],'indexUrl':f'./{ip.name}?v=2'}
out={'version':2,'status':'ready','engine':'aivisspeech-style-bert-vits2','storage':'github-releases+hf-range-bundles','wordCount':word_count,'voiceCount':len(voices),'recordingCount':word_count*len(voices),'shardCount':sc,'model':{'uuid':model['modelUuid'],'name':model['modelName'],'version':model['modelVersion'],'architecture':model['modelArchitecture'],'license':model['license'],'licenseSha256':model['licenseSha256']},'words':words,'voices':voices}
