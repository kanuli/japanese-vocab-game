...
TIMES=['八時','八時半','九時','九時半','十時半','十一時半','一時','一時半','二時','三時']
ALT_TIMES=['七時半','八時','八時半','九時','九時半','十時','十時半','十一時','十二時','一時','二時','四時']
DAYS=['明日','あさって','月曜日','火曜日','水曜日','木曜日','土曜日','日曜日','来週の月曜日','来週の水曜日']
...
REASONS=['電車が遅れている','担当者が席を外している','雨が強くなった','予約が全部埋まっている','会議が長引いている','必要な資料がまだ届いていない','体調がよくない','工事で入口が使えない','予定が変更された','機械の点検が必要だ','道路が混んでいる','参加者が増えた']
...
def combo(i,*seqs):
    if len(seqs)==8:
        raw=(i//100,i,i//10,i//10,i//120,i,i,i//120)
        return [seq[n%len(seq)] for seq,n in zip(seqs,raw)]
    out=[]
    for seq in seqs:
