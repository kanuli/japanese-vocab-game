...
function core(rows){const out=[];for(let r of rows){if(!r.length||String(r[0]).startsWith("#"))continue;if(r.length===F.length+1)r=r.slice(1);if(r.length!==F.length)continue;const f=Object.fromEntries(F.map((n,j)=>[n,r[j]||""])),w=display(f.VocabKanji),m=strip(f.VocabDefTC),lv=`${f.Deck} ${f.Tags}`.match(/(?:^|[^A-Za-z0-9])N([1-5])(?=$|[^0-9])/i),rd=reading(f.VocabFurigana,w);if(w&&m&&lv&&rd){const key=`${rd}|${w}`,level=exactLevel[key]||`N${lv[1]}`;out.push({id:`c-${f.NoteID||out.length}`,level,reading:rd,kanji:kanji(w)?w:"",displayWord:w,meaning:m,estimated:false,teacherGrade:"core",teacherBasis:exactLevel[key]?"teacher-exact-display-correction":"external-core-fetch"})}}return out}
