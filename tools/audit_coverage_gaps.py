...
GRAMMAR_MATRIX = {
'N5': {
 'request':['てください'], 'permission':['てもいい'], 'prohibition':['てはいけない','てはいけません'],
 'desire':['たい'], 'invitation':['ませんか'], 'proposal':['ましょう'],
 'reason':['から'], 'sequence':['てから'], 'examples':['たり']},
'N4': {
 ...
 'potential':['可能形','potential'], 'passive':['受身','受け身','passive']},
'N3': {
 'decision':['ことにする'], 'external_decision':['ことになる'], 'rule':['ことになっている'],
 'habit_effort':['ようにする','ようにしています'], 'change_state':['ようになる'], 'every_time':['たびに'],
...
JF_TOPICS = {
 'self_family':['家族','自己紹介','名前','結婚','子育て','家庭'],
 'housing':['部屋','住宅','不動産','引っ越','水道','電気','ガス','ごみ','近所','住まい'],
 'leisure':['映画','音楽','スポーツ','趣味','イベント','コンサート','娯楽','劇場'],
 'life_lifecycle':['生活','日課','役所','行政','手続','冠婚葬祭','暮らし'],
 'work':['仕事','会社','職場','会議','商務','報告','顧客','上司','勤務'],
 'travel_transport':['旅行','駅','電車','バス','空港','飛行機','船','フェリー','タクシー','交通','観光'],
 'health':['病院','薬','健康','診察','医療','歯科','クリニック'],
 'shopping':['買い物','返品','交換','支払','店','商品','スーパー','コンビニ'],
 'food':['レストラン','注文','料理','食事','カフェ','飲食','食べ物','アレルギー','餐廳','咖啡','飲食'],
 'nature_environment':['天気','災害','避難','自然','環境','台風','地震','大雨','防災','天氣'],
 'relationships':['友達','人間関係','謝罪','相談','トラブル','近所','交際'],
 'education':['学校','大学','授業','勉強','図書館','教育','教室'],
 'language_culture':['日本語','文化','神社','寺','祭り','言語','マナー','伝統','語言','文化'],
 'society':['警察','社会','銀行','保険','入管','法律','行政','市役所'],
 'science_technology':['技術','スマホ','インターネット','アカウント','パソコン','オンライン','システム']
}

FUNCTIONS = {
 'request':['ください','お願いします','いただけます'],
 'permission':['てもいい','可能ですか','よろしいですか'],
 'apology':['すみません','申し訳'],
 'refusal':['できません','難しい','お断り','いたしかね'],
 'clarification':['確認したい','ということですか','もう一度','聞き逃'],
 'change_reschedule':['変更','取り直','改めて','別の時間','振り替'],
 'complaint_problem':['困って','問題','故障','届いていない','違う','不具合'],
 'negotiation_alternative':['別の方法','代わり','調整','対応していただく','ほかの方法'],
 'procedure':['手続','必要書類','受付','申請','提出'],
 'reporting':['報告','連絡','共有','伝えて'],
 'emergency':['緊急','避難','事故','警察','救急','災害'],
 'reason_explanation':['ので','ため','理由','事情','から'],
 'exception':['例外','通常の方法以外','事情がある場合','今回に限り'],
 'recommendation':['ほうがいい','おすすめ','ましょう','したほう'],
 'comparison':['比べ','より','一方','ほうが']
}
FUNCTION_EXPECTED = {
 'N5':['request','permission','apology','clarification'],
 'N4':['request','permission','apology','clarification','change_reschedule','reason_explanation','recommendation'],
 'N3':['request','apology','clarification','change_reschedule','complaint_problem','procedure','reporting','reason_explanation','recommendation','comparison'],
 'N2':['request','apology','refusal','clarification','change_reschedule','complaint_problem','negotiation_alternative','procedure','reporting','reason_explanation','exception','recommendation','comparison'],
 'N1':['request','apology','refusal','clarification','change_reschedule','complaint_problem','negotiation_alternative','procedure','reporting','reason_explanation','exception','comparison']
}
...
def collect_grammar():
    ...
    for level, gram in re.findall(r"add\('(N[1-5])','([^']+)'", e): per[level].append(gram)
    remote_status = {}
    ...
    for level in LEVELS:
        ...
        else: remote_status[level] = d.get('_error','unknown')
...
def grammar_audit():
    ...
    for level in LEVELS:
        ...
        for fn, aliases in GRAMMAR_MATRIX[level].items():
            (covered if any(norm(a) in hay for a in aliases) else missing).append(fn)
...
def parse_reference_listening():
    ...
    for level in LEVELS:
        ...
        if not m: continue
        for jp,zh,t in re.findall(r"\['([^']*)','([^']*)','([^']*)'\]",m.group(1)):
            rows.append({'level':level,'typeZh':t,'jp':jp})
    return rows

def listening_audit():
    d=json.loads(read('listening-original-catalog.json'))
    rows=[x for x in d.get('items',[]) if x.get('level') in LEVELS] + parse_reference_listening()
    out={}
    for level in LEVELS:
        r=[x for x in rows if x.get('level')==level]
        tc=Counter(str(x.get('typeZh') or x.get('type') or '未分類') for x in r)
        out[level]={'count':len(r),'distinctTypes':len(tc),'topTypes':tc.most_common(15),'underrepresented':sorted([[k,v] for k,v in tc.items() if v<8],key=lambda x:(x[1],x[0]))[:30]}
    return out
...
def conversation_audit():
    ...
    for m in re.finditer(r'\{"id":"([^"]+)","icon":"[^"]*","zh":"([^"]+)","jp":"([^"]+)"',text): scenes.append({'id':m.group(1),'zh':m.group(2),'jp':m.group(3)})
    ...
    level_function={}
    for level in LEVELS:
        json_snips=re.findall(rf'\{{"level":"{level}".*?"lines":\[.*?\]\}}',text,re.S)
        generated_snips=re.findall(rf"item\('{level}'.{{0,1000}}?\)\s*[,;]",text,re.S)
        local=' '.join(json_snips+generated_snips)
        counts={fn:sum(local.count(k) for k in kws) for fn,kws in FUNCTIONS.items()}
        weak=[fn for fn in FUNCTION_EXPECTED[level] if counts.get(fn,0)<3]
        level_function[level]={'counts':counts,'weakExpected':weak}
    weak_topics=[t for t in JF_TOPICS if topic_counts[t]<2]
    return {'sceneCount':len(scenes),'topicCounts':dict(topic_counts),'weakTopics':weak_topics,'levelFunctions':level_function,'sceneTopics':scene_topics}

def main():
    grammar,remote=grammar_audit();listening=listening_audit();conversation=conversation_audit()
    report={
      'version':'2026-08-27-gap-audit-v2',
      'methodology':{
    ...
    print(json.dumps({'report':str(p.relative_to(ROOT)),'grammarMissing':{k:v['missing'] for k,v in grammar.items()},'weakTopics':conversation['weakTopics'],'weakFunctions':{k:v['weakExpected'] for k,v in conversation['levelFunctions'].items()}},ensure_ascii=False))
