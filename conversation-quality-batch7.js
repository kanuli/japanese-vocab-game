...
let rewritten=0,duplicateFixes=0;
...
for(const s of scenes){
 (s.items||[]).forEach((x,i)=>{
  ...
  let m;
  if((m=String(a.jp).match(/^すみません。(.+?)はどこでできますか。$/))){a.jp=worldA.N5(m[1],i);rewritten++;}
  else if((m=String(a.jp).match(/^(.+?)には何が必要ですか。$/))){a.jp=worldA.N4(m[1],i);rewritten++;}
  else if((m=String(a.jp).match(/^(.+?)について確認したいのですが、「(.+?)」という状況でも大丈夫ですか。$/))){a.jp=worldA.N3(m[1],m[2],i);rewritten++;}
  else if((m=String(a.jp).match(/^「(.+?)」という状況なのですが、(.+?)を進めるにはどうすればいいですか。$/))){a.jp=worldA.N2(m[2],m[1],i);rewritten++;}
  ...
  if(s.referenceExpansion){
    if((m=String(a.jp).match(/^(.+?)をお願いします。$/))){const n=m[1];a.jp=pick([`${n}をお願いします。`,`${n}をお願いできますか。`,`${n}を利用したいです。`,`${n}についてお願いしたいです。`,`${n}を希望しています。`],s.id,i);b.jp=pick(['はい、承知しました。','わかりました。確認します。','はい、対応します。','かしこまりました。','はい、ご案内します。'],s.id,i);rewritten++;}
    else if((m=String(a.jp).match(/^(.+?)をしたいです。何を準備したらいいですか。$/))){const n=m[1];a.jp=pick([`${n}をしたいです。何を用意すればいいですか。`,`${n}には何が必要ですか。`,`${n}の準備物を教えてください。`,`${n}の前に何を準備すればいいですか。`,`${n}をするために必要なものは何ですか。`],s.id,i);rewritten++;}
    else if((m=String(a.jp).match(/^(.+?)ので、(.+?)について確認したいのですが、どうすればいいですか。$/))){const problem=m[1],n=m[2];a.jp=pick([`${problem}ので、${n}について確認したいです。どうすればいいですか。`,`${n}について質問があります。${problem}場合はどうすればいいですか。`,`${problem}ので、${n}の対応方法を教えてください。`,`${n}を確認したいです。${problem}ときは何をすればいいですか。`,`${problem}という状況です。${n}について注意する点はありますか。`],s.id,i);rewritten++;}
    else if((m=String(a.jp).match(/^(.+?)という状況です。(.+?)を進める上で、別の方法はありますか。$/))){const problem=m[1],n=m[2];a.jp=pick([`${problem}という状況です。${n}について別の方法はありますか。`,`${n}について相談です。${problem}場合の対応方法を教えてください。`,`${problem}ので、${n}について利用できる別の方法を確認したいです。`,`${n}について、${problem}という問題があります。ほかの対応はできますか。`,`${problem}場合、${n}はどのように対応すればいいでしょうか。`],s.id,i);const bm=String(b.jp).match(/^(.+?)。条件によって必要な手続きが変わる場合があります。$/);if(bm)b.jp=pick([`${bm[1]}。条件を確認すると、必要な手続きがわかります。`,`${bm[1]}。状況によって手順が異なるので、確認して進めてください。`,`${bm[1]}。必要な手続きは条件ごとに確認できます。`,`${bm[1]}。詳しい条件を確認してから手続きをご案内します。`,`${bm[1]}。場合によって手順が変わるため、先に条件を確認してください。`],s.id,i);rewritten++;}
  ...
  if(rewritten)x.qualityDepthBatch7=x.qualityDepthBatch7||false;
 });
}

// Resolve the three exact duplicate base-dialogue pairs without changing their communicative function.
const dupFixes=[
...
// Mark rewritten items after transformations by comparing against the patterns now gone.
for(const s of scenes)for(const x of s.items||[])if((x.lines||[]).some(l=>String(l.jp||'').includes('通常とは')||String(l.jp||'').includes('対応方法を確認したい')||String(l.jp||'').includes('準備物を教えて')))x.qualityDepthBatch7=true;
window.CONVERSATION_QUALITY_BATCH7={rewritten,duplicateFixes};
})();
